...
import { Search,Toolbar,GridComponent, ColumnsDirective, ColumnDirective, Inject, Filter, VirtualScroll, Sort } from '@syncfusion/ej2-react-grids';
...
function trustTemplate(props) {
    ...
    var Trustworthiness = props.Trustworthiness === "Sufficient" ? require("./grid/images/Sufficient.png") : props.Trustworthiness === "Insufficient" ? require("./grid/images/Insufficient.png") : require("./grid/images/Perfect.png");
...
function empTemplate(props) {
    ...
    {props.EmployeeImg === 'usermale' ?
            <div className="empimeg">
          <img src={require("./grid/images/usermale.png")} style={{width:"30px",alignItems:"center",marginTop:"8px"}}/>
        </div> :
            <div className="empimeg">
          <img src={require("./grid/images/userfemale.png")} style={{width:"30px",alignItems:"center",marginTop:"8px"}}/>
        </div>}
...
function coltemplate(props) {
    ...
    <img src={require("./grid/images/Map.png")} className="e-image"/> <span>  </span>
...
function App() {
    ...
    return (<div className='control-pane' style={{marginTop:"5%"}}>
      <div className='control-section'>
        ...
        <GridComponent data-testid='my-link' id="overviewgrid" toolbar={toolbarOptions} dataSource={data} loadingIndicator={{ indicatorType: 'Shimmer' }} query={query} enableHover={false} enableVirtualization={true} rowHeight={38} height='400' ref={(g) => { gridInstance = g; }} actionComplete={onComplete.bind(this)} load={onLoad.bind(this)} dataBound={onDataBound.bind(this)} filterSettings={gridFilter} allowFiltering={true} allowSorting={true} allowSelection={true} selectionSettings={select} enableHeaderFocus={true}>
          <ColumnsDirective>
            <ColumnDirective id="column" type='checkbox' allowSorting={false} allowFiltering={false} width='60'></ColumnDirective>
            <ColumnDirective id="column" field='EmployeeID' visible={false} headerText='Employee ID' isPrimaryKey={true} width='130'/>
            <ColumnDirective id="column" field='Employees' headerText='Employee Name' width='280' allowFiltering={true} clipMode='EllipsisWithTooltip' template={empTemplate}/>
            <ColumnDirective id="column" field='Designation' headerText='Designation' width='170' allowFiltering={true} clipMode='EllipsisWithTooltip'/>
            <ColumnDirective id="column" field='Mail' headerText='Mail' width='230' allowFiltering={true} ></ColumnDirective>
            <ColumnDirective id="column" field='Location' headerText='Location' width='140' template={coltemplate} allowFiltering={true}></ColumnDirective>
            <ColumnDirective id="column" field='Status' headerText='Status' template={statusTemplate} width='130' allowFiltering={true}></ColumnDirective>
            <ColumnDirective id="column" field='Trustworthiness' headerText='Trustworthiness' template={trustTemplate} width='160' allowFiltering={true}></ColumnDirective>
            <ColumnDirective id="column" field='Rating' headerText='Rating' template={ratingTemplate} width='220'/>
            <ColumnDirective id="column" field='Software' allowFiltering={false} allowSorting={false} headerText='Software Proficiency' width='180' template={progessTemplate} format='C2'/>
            <ColumnDirective id="column" field='CurrentSalary' headerText='Current Salary' width='160' format='C2'></ColumnDirective>
            <ColumnDirective id="column" field='Address' headerText='Address' width='240' clipMode="EllipsisWithTooltip"></ColumnDirective>
          </ColumnsDirective>
          <Inject services={[Filter, VirtualScroll, Sort,Toolbar,Search]}/>
        </GridComponent>
      </div>
      <style>
        @import 'src/grid/style.css';
      </style>
    </div>);
}
export default App;